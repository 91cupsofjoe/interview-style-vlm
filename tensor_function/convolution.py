...
KERNEL_SIZE = (3, 3)
STRIDE = 1
_PADDING_H = int((KERNEL_SIZE[0] - 1) / 2)
_PADDING_W = int((KERNEL_SIZE[1] - 1) / 2)
...
def _conv2d(
    ...
    # Make sure the padding is a tuple
    padding = util._get_tuple(padding)
    ...
    patch_h = math.floor( (input_h + 2 * padding_h - kernel_h) / stride) + 1
    patch_w = math.floor( (input_w + 2 * padding_w - kernel_w) / stride) + 1
    ...
    if b is not None:
        x_proj += b.view(1, -1, 1) # b.view(1, num out features, 1)
