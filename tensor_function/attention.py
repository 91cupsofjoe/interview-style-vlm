...
from tensor_function.activation import _softmax, _softmax_backward
from tensor_function.mask import _get_causal_mask, _get_padding_mask
from tensor_function.projection import _lin_proj, _lin_proj_backward
from tensor_function.reshape import _split_tensor
...
def _scaled_dot_product_attention_backward(
    ...
    scaled_scores_grad = _softmax_backward(
...
def _multi_head_attention(
    ...
    Q_proj = _lin_proj(x=Q, W=W_q, b=b_q)
    K_proj = _lin_proj(x=K, W=W_k, b=b_k)
    V_proj = _lin_proj(x=V, W=W_v, b=b_v)

    # Then split the projected matrices
    Q_split = _split_tensor(x=Q_proj, num_split_heads=num_attn_heads)
    K_split = _split_tensor(x=K_proj, num_split_heads=num_attn_heads)
    V_split = _split_tensor(x=V_proj, num_split_heads=num_attn_heads)
    ...
    #   projected matrices
    context_vector, attention_weights = _scaled_dot_product_attention(
    ...
    context_vector = _lin_proj(x=context_vector, W=W_o, b=b_o)
...
def _multi_head_attention_backward(
    ...
    O_in_grad, W_o_grad, b_o_grad = _lin_proj_backward(
    ...
    Q_heads_grad, K_heads_grad, V_heads_grad = _scaled_dot_product_attention_backward(
    ...
    Q_in_grad, W_q_grad, b_q_grad = _lin_proj_backward(
    ...
    K_in_grad, W_k_grad, b_k_grad = _lin_proj_backward(
    ...
    V_in_grad, W_v_grad, b_v_grad = _lin_proj_backward(
...
def _multi_head_masked_self_attention(
    Q: Tensor, K: Tensor, V: Tensor,
    W_q: Tensor, W_k: Tensor, W_v: Tensor, W_o: Tensor,
    num_attn_heads=NUM_ATTN_HEADS,
    b_q: Optional[Tensor]=None, b_k: Optional[Tensor]=None,
    ...
    NOTE: See _multi_head_attention() for the full implementation details.
    ...
    if causal_mask is not None and padding_mask is not None:
        ...
        return _multi_head_attention(**kwargs)
    
    # Else return None


def _multi_head_cross_attention(
    Q: Tensor, K: Tensor, V: Tensor,
    W_q: Tensor, W_k: Tensor, W_v: Tensor, W_o: Tensor,
    num_attn_heads=NUM_ATTN_HEADS,
    b_q: Optional[Tensor]=None, b_k: Optional[Tensor]=None,
