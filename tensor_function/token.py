...
def _get_token_ids(
    ...
    sentence_lists = _get_sentence_lists(
        sentence=sentence,
        sentences=sentences,
    ...
    for sentence_list in sentence_lists:
        ...
        for sentence in sentence_list:
            ...
            tokens_tuple = _parse_sentence_into_tuple(
                sentence=sentence,
...
def _parse_sentence_into_tuple(
    sentence: str,
...
def _get_positional_encodings(
    ...
    return _get_sinusoidal_encodings(shape[-2], shape[-1])


def _get_embedded_tokens(
    tokens: Tensor,
    ...
    if token_embeddings is None:
        ...
        if token_ids is not None and embedding_size is not None:
            token_embeddings = _get_token_embeddings(token_ids, embedding_size)
    ...
    if use_positional_encodings and positional_encodings is None:
        positional_encodings = _get_positional_encodings(
            shape=tokens.shape,
...
def _get_embedded_tokens_backward(
