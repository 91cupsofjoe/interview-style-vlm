    ...
    def __init__(self, vocab_size, d_model, max_seq_len):
        ...
        for pos in range(max_seq_len):
            for dim in range(d_model):
                ...
                # PE(pos, 2i + 1) = cos( pos / (e^(2i/d_model)*ln(10000)) )
                base_value = pos / (math.pow(math.e, 2*dim/d_model)) * math.log(10000)
                if dim % 2 == 0:
                    pe[pos, dim] = math.sin(base_value)
                else:
                    pe[pos, dim] = math.cos(base_value)
...
def scaled_dot_product_attention(Q, K, V, mask=None):
    ...
    if mask is not None:
        scaled_scores += mask
    # weights dims after softmax = [T]
    attention_weights = torch.softmax(scaled_scores, dim=3)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model, num_heads):
        ...
        self.proj_o = nn.Linear(self.d_model, self.d_model, bias=False)
    ...
    def split_heads(self, x):
        ...
        return x.transpose(1, 2) # Transpose seq_len and num_heads
    ...
    def forward(self, x_q, x_k, x_v, mask=None):
        ...
        context_vector, _ = scaled_dot_product_attention(
                        Q=Q, K=K, V=V, mask=mask)
        
        # 4. Combine Heads
        #    a. Reverse the 'split_heads' operation (.transpose and .contiguous)
        # TODO: Implement this (get shape and .view)
        # Transpose seq_len and num_heads to get [batch_size, seq_len, num_heads, d_head]
        context_vector = context_vector.transpose(1, 2)
        # Flatten the last two dimensions into one
        batch_size, seq_len = context_vector.shape[:-2]
        context_vector = context_vector.reshape(batch_size, seq_len, -1)
        
        # 5. Final Linear Layer (W_o)
        # TODO: Implement this
        return self.proj_o(context_vector)
...
class TransformerEncoderBlock(nn.Module):
    ...
    def __init__(self, d_model, num_heads, d_ff, dropout=0.1):
        ...
        self.layernorm1 = torch.layer_norm
        self.layernorm2 = torch.layer_norm
        self.dropout1 = torch.dropout
        self.dropout2 = torch.dropout
    ...
    def forward(self, x, mask=None):
        ...
        # TODO: Implement this
        mha_output = self.mha.forward(
            x_q=x, x_k=x, x_v=x, mask=mask)
        
        x_with_mha = self.layernorm1(
            normalized_shape=mha_output.shape,
            input=(x + self.dropout1(
                input=mha_output, p=self.ffn.dropout, train=True
            ))
        )
        
        # 2. FFN + "Add & Norm"
        # TODO: Implement this
        ffn_output = self.ffn.forward(x=x_with_mha)

        block_output = self.layernorm2(
            normalized_shape=ffn_output.shape,
            input=(x_with_mha + self.dropout2(
                input=ffn_output, p=self.ffn.dropout, train=True
            ))
        )
...
class TransformerDecoderBlock(nn.Module):
    ...
    def __init__(self, d_model, num_heads, d_ff, dropout=0.1):
        ...
        self.cross_mha = MultiHeadAttention(d_model=d_model, num_heads=num_heads)
        self.ffn = PositionwiseFeedForward(d_model=d_model, d_ff=d_ff, dropout=dropout)
        self.layernorm1 = torch.layer_norm
        self.layernorm2 = torch.layer_norm
        self.layernorm3 = torch.layer_norm
        self.dropout1 = torch.dropout
        self.dropout2 = torch.dropout
        self.dropout3 = torch.dropout
    ...
    def forward(self, x, encoder_output, causal_mask, src_padding_mask=None):
        ...
        mha_output = self.masked_mha.forward(
            x_q=x, x_k=x, x_v=x,
            mask=combined_mask)
        ...
        # TODO: Implement this
        cross_output = self.masked_mha.forward(
            x_q=x_with_mha, x_k=encoder_output, x_v=encoder_output,
            mask=src_padding_mask)
        
        x_with_cross = self.layernorm2(
            normalized_shape=cross_output.shape,
            input=(x_with_mha + self.dropout2(
                input=cross_output,
                p=self.ffn.dropout,
                train=True
        ...
        # 3. FFN + "Add & Norm"
        # TODO: Implement this
        ffn_output = self.ffn.forward(x=x_with_cross)

        block_output = self.layernorm3(
            normalized_shape=ffn_output.shape,
            input=(x_with_cross + self.dropout3(
                input=ffn_output,
                p=self.ffn.dropout,
                train=True
            ))
